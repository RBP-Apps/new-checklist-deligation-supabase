    ...
    const fetchData = async () => {
        try {
            ...
            const [holidaysRes, workingDaysRes, checklistRes, maintenanceRes, delegationRes, eaRes] = await Promise.all([
                supabase.from('new_holidays').select('*'),
                supabase.from('new_working_day_calender')
                    .select('*')
                    .gte('working_date', startOfMonth)
                    .lte('working_date', endStr)
                    .order('working_date', { ascending: true }),
                supabase.from('new_checklist').select('planned_date, task_start_date').gte('planned_date', startOfMonth).lte('planned_date', endStr),
                supabase.from('new_maintenance_tasks').select('planned_date').gte('planned_date', startOfMonth).lte('planned_date', endStr),
                supabase.from('new_delegation').select('planned_date').gte('planned_date', startOfMonth).lte('planned_date', endStr),
                supabase.from('new_ea_tasks').select('planned_date').gte('planned_date', startOfMonth).lte('planned_date', endStr)
            ]);
    ...
    const toggleWorkingDay = async (dateStr, isWorking, isHoliday) => {
        ...
        try {
            ...
            if (isWorking) {
                ...
                await Promise.all([
                    supabase.from('new_checklist').delete().gte('task_start_date', startOfDay).lte('task_start_date', endOfDay),
                    supabase.from('new_delegation').delete().gte('task_start_date', startOfDay).lte('task_start_date', endOfDay),
                    supabase.from('new_maintenance_tasks').delete().gte('task_start_date', startOfDay).lte('task_start_date', endOfDay),
                    supabase.from('new_ea_tasks').delete().gte('planned_date', startOfDay).lte('planned_date', endOfDay)
                ]);
