        ...
        const updates = await Promise.all(submissionData.map(async (item) => {
            ...
            if (item.image && item.image.previewUrl) {
                try {
                    ...
                    const { error: uploadError } = await supabase.storage.from('new_maintenance').upload(fileName, file);

                    if (uploadError) throw uploadError;

                    const { data: { publicUrl } } = supabase.storage.from('new_maintenance').getPublicUrl(fileName);
                    imageUrl = publicUrl;
...
export const updateMaintenanceTaskApi = async (updatedTask, originalTask) => {
    try {
        let query = supabase.from("new_maintenance_tasks").update({
