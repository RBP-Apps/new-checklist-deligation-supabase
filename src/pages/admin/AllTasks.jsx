...
const AllTasks = () => {
    ...
    const fetchInitialData = async () => {
      try {
        const [holidaysRes, usersRes, workingDaysRes] = await Promise.all([
          supabase.from('new_holidays').select('holiday_date'),
          supabase.from('new_users').select('user_name').eq('status', 'active').order('user_name', { ascending: true }),
          supabase.from('new_working_day_calender').select('working_date')
        ]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      switch (activeTab) {
        case "maintenance":
          tableName = "new_maintenance_tasks";
      ...
      if (showHistory) {
        if (activeTab === "repair") {
          ...
          query = supabase.from("new_ea_tasks_done").select("*").order("created_at", { ascending: false });
        } else {
  ...
  const uploadFile = async (id, file) => {
    ...
    const resolvedBucket = bucketName === 'checklist' ? 'new_checklist' :
                           bucketName === 'delegation' ? 'new_checklist' :
                           bucketName === 'maintenance' ? 'new_maintenance' :
                           bucketName === 'repair' ? 'new_repair' :
                           bucketName === 'ea' ? 'new_ea' :
                           bucketName;
    const fileName = `${id}_${Date.now()}_${file.name}`;
    const { data, error: uploadError } = await supabase.storage.from(resolvedBucket).upload(fileName, file);
 
    if (uploadError) throw uploadError;
 
    const { data: { publicUrl } } = supabase.storage.from(resolvedBucket).getPublicUrl(fileName);
    return publicUrl;
  };
  ...
  const handleRepairUpdateSubmit = async (e) => {
    ...
    try {
      ...
      if (updateForm.workPhoto) {
        ...
        const { data, error } = await supabase.storage.from('new_repair').upload(fileName, updateForm.workPhoto);
        if (error) throw error;
        const { data: { publicUrl } } = supabase.storage.from('new_repair').getPublicUrl(fileName);
      ...
      if (updateForm.billCopy) {
        ...
        const { data, error } = await supabase.storage.from('new_repair').upload(fileName, updateForm.billCopy);
        if (error) throw error;
        const { data: { publicUrl } } = supabase.storage.from('new_repair').getPublicUrl(fileName);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const tableName = activeTab === "checklist" ? "new_checklist" :
        activeTab === "delegation" ? "new_delegation" :
          activeTab === "maintenance" ? "new_maintenance_tasks" :
            activeTab === "ea" ? "new_ea_tasks" :
              "new_repair_tasks";
      const completionField = "submission_date";
      ...
      const updatePromises = selectedArray.map(async (id) => {
        ...
        if (activeTab === "ea") {
          ...
          if (isExtendStatus && extendedDateData[id]) {
            ...
            const { error: doneError } = await supabase.from("new_ea_tasks_done").insert([{
              task_id: id,
            ...
            // 2. Update ea_tasks
            const { error: updateError = null } = await supabase.from("new_ea_tasks").update({
