...
export const createUserApi = async (newUser) => {
  try {
    ...
    if (error && (error.code === 'PGRST204' || error.message?.includes('Designation') || error.code === '42703')) {
      ...
      const retry = await supabase.from("new_users").insert([fallbackData]).select().maybeSingle();
      data = retry.data;
      error = retry.error;
    }
...
export const updateUserDataApi = async ({ id, updatedUser }) => {
  try {
    ...
    if (error && (error.code === 'PGRST204' || error.code === '42703' || error.message?.toLowerCase().includes('Designation'.toLowerCase()))) {
      ...
      const retry = await supabase.from("new_users").update(fallbackData).eq("id", id).select().maybeSingle();
      data = retry.data;
      error = retry.error;
    }
...
export const deleteCustomDropdownApi = async (id) => {
  try {
    const { error } = await supabase.from('new_dropdown_options').delete().eq('id', id);
    if (error) throw error;
    return id;
  } catch (error) {
