...
export default function EATask() {
    ...
    const fetchUniqueDoers = async () => {
        try {
            const { data, error } = await supabase.from('new_ea_tasks').select('doer_name, phone_number').order('created_at', { ascending: false });
    ...
    const handleSubmitAll = async () => {
        ...
        try {
            ...
            for (let i = 0; i < tasksToInsert.length; i += CHUNK_SIZE) {
                ...
                const { data, error } = await supabase.from('new_ea_tasks').insert(chunk).select();
                if (error) throw error;
