...
const isAudioUrl = (url) => {
    ...
    return url.startsWith('http') && (
        ...
        url.includes('new_audio-recordings') ||
            ...
            <div className="p-5 space-y-4">
                ...
                <div>
                    ...
                    <select
                        ...
                        value={task.filledBy}
                        disabled={true}
                        className="w-full p-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none bg-gray-50 focus:bg-white transition-all text-sm opacity-70 cursor-not-allowed"
                    >
                        <option value={task.filledBy}>{task.filledBy}</option>
                    </select>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <SearchableSelect
                        ...
                        onChange={(val) => {
                            onUpdate(task.id, { machineName: val });
                        }}
                    />
                    <SearchableSelect
                        label="Assign To"
                        ...
                        options={getFilteredDoers().map(user => user.user_name)}
                        onChange={(val) => {
                            onUpdate(task.id, { assignedPerson: val });
                        }}
                    />
                </div>
...
export default function RepairTask() {
    ...
    useEffect(() => {
        const fetchHolidaysAndWorkingDays = async () => {
            ...
            // Fetch holidays
            const { data: holidayData } = await supabase.from('new_holidays').select('holiday_date');
    ...
    const handleSubmitAll = async () => {
        ...
        try {
            ...
            for (let i = 0; i < requestsToInsert.length; i += CHUNK_SIZE) {
                ...
                const { data, error } = await supabase.from('new_repair_tasks').insert(chunk).select();
