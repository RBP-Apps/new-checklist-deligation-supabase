...
const CalendarPage = () => {
    ...
    const fetchTasks = async () => {
        try {
            ...
            let checklistQuery = supabase.from('new_checklist').select('*').gte('planned_date', startStr).lte('planned_date', endStr);
            let maintenanceQuery = supabase.from('new_maintenance_tasks').select('*').gte('planned_date', startStr).lte('planned_date', endStr);
            let repairQuery = supabase.from('new_repair_tasks').select('*').gte('created_at', startStr).lte('created_at', endStr);
            let delegationQuery = supabase.from('new_delegation').select('*').gte('planned_date', startStr).lte('planned_date', endStr);
            let eaQuery = supabase.from('new_ea_tasks').select('*').gte('planned_date', startStr).lte('planned_date', endStr);
