...
export const pushAssignTaskApi = async (generatedTasks, targetTable = null) => {
  ...
  if (targetTable) {
    ...
    try {
      const resolvedTable = targetTable === 'checklist' ? 'new_checklist' : (targetTable === 'delegation' ? 'new_delegation' : targetTable);
      const { data, error } = await supabase.from(resolvedTable).insert(tasksData).select();
      if (error) throw error;
      return data;
    } catch (error) {
  ...
  try {
    if (delegationTasks.length > 0) {
      const { data, error } = await supabase.from('new_delegation').insert(delegationTasks).select();
      if (error) {
    ...
    if (checklistTasks.length > 0) {
      const { data, error } = await supabase.from('new_checklist').insert(checklistTasks).select();
      if (error) {
