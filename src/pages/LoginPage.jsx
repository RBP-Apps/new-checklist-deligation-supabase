              ...
              <div className="px-6 pb-8 space-y-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          const { data, error } = await supabase.from('new_users').select('user_name').eq('user_name', forgotData.username).single();
                ...
                {forgotStep === 'reset' && (
                  <form onSubmit={async (e) => {
                    ...
                    setIsForgotLoading(true);
                    try {
                      const { error } = await supabase.from('new_users').update({ password: forgotData.newPassword }).eq('user_name', forgotData.username);
