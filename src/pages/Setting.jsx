...
const Setting = () => {
  ...
  const handleFetchLeaveTasks = async () => {
    ...
    try {
      ...
      const [
        ...
        supabase.from('new_checklist').select('*').eq('name', leavePersonName)
          .gte('task_start_date', startISO).lte('task_start_date', endISO).is('submission_date', null),
        supabase.from('new_delegation').select('*').eq('name', leavePersonName)
          .gte('task_start_date', startISO).lte('task_start_date', endISO).is('submission_date', null),
        supabase.from('new_maintenance_tasks').select('*').eq('name', leavePersonName)
          .gte('task_start_date', startISO).lte('task_start_date', endISO).is('submission_date', null),
        supabase.from('new_repair_tasks').select('*').eq('assigned_person', leavePersonName)
          .gte('created_at', startISO).lte('created_at', endISO).eq('status', 'Pending'),
        supabase.from('new_ea_tasks').select('*').eq('doer_name', leavePersonName)
  ...
  const handleShiftTasks = async () => {
    ...
    try {
      ...
      const { data: currentUser } = await supabase.from('new_users').select('status').eq('id', leavePersonId).single();
      ...
      if (checklistIds.length > 0) {
        const { error: checklistError } = await supabase.from('new_checklist').update({ name: shiftToPerson }).in('task_id', checklistIds);
        if (checklistError) console.error('Error updating checklist tasks:', checklistError);
      }
      if (delegationIds.length > 0) {
        const { error: delegationError } = await supabase.from('new_delegation').update({ name: shiftToPerson }).in('task_id', delegationIds);
        if (delegationError) console.error('Error updating delegation tasks:', delegationError);
      }
      if (maintenanceIds.length > 0) {
        const { error: maintenanceError } = await supabase.from('new_maintenance_tasks').update({ name: shiftToPerson }).in('id', maintenanceIds);
        if (maintenanceError) console.error('Error updating maintenance tasks:', maintenanceError);
      }
      if (repairIds.length > 0) {
        const { error: repairError } = await supabase.from('new_repair_tasks').update({ assigned_person: shiftToPerson }).in('id', repairIds);
        if (repairError) console.error('Error updating repair tasks:', repairError);
      }
      if (eaIds.length > 0) {
        const { error: eaError } = await supabase.from('new_ea_tasks').update({ doer_name: shiftToPerson }).in('id', eaIds);
  ...
  const handleAddUser = async (e) => {
    ...
    if (!userForm.employee_id || userForm.employee_id.trim() === "") {
      showToast("Employee ID is required", "error");
      return;
    }

    const empIdExists = userData.some(u => u.employee_id && u.employee_id.toLowerCase() === userForm.employee_id.trim().toLowerCase());
    if (empIdExists) {
    ...
    if (profileFile) {
      try {
        imageUrl = await dispatch(uploadProfileImage({ file: profileFile, userId: userForm.employee_id.trim() })).unwrap();
      } catch (uploadErr) {
  ...
  const handleUpdateUser = async (e) => {
    ...
    const empIdExists = userData.some(u => u.id !== currentUserId && u.employee_id && u.employee_id.toLowerCase() === userForm.employee_id.trim().toLowerCase());
    if (empIdExists) {
      showToast("Employee ID already exists", "error");
      return;
    }
    ...
    if (profileFile) {
      try {
        imageUrl = await dispatch(uploadProfileImage({ file: profileFile, userId: userForm.employee_id.trim() || currentUserId })).unwrap();
      } catch (uploadErr) {
        console.error('Image upload failed:', uploadErr);
    ...
    try {
      ...
      if (userName) {
        const deletePromises = [
          supabase.from('new_checklist').delete().eq('name', userName),
          supabase.from('new_delegation').delete().eq('name', userName),
          supabase.from('new_maintenance_tasks').delete().eq('name', userName),
          supabase.from('new_repair_tasks').delete().eq('assigned_person', userName),
          supabase.from('new_ea_tasks').delete().eq('doer_name', userName)
        ];
